...
const API_BASE_URL = 'http://14.139.187.54:443/api'
...
export async function fetchFilterOptions() {
    try {
        ...
        console.log(`Fetching filter options from ${API_BASE_URL}/filter-options`)
...
export async function fetchStudentData(filters: { academicYear: string, course: string, semester: string }) {
    try {
        ...
        const url = `${API_BASE_URL}/admitted-students?${queryParams.toString()}`
