...
export default function PracticalAssessmentPage() {
    ...
    const handleExport = () => {
        ...
        if (link.download !== undefined) {
            const url = URL.createObjectURL(blob);
            link.setAttribute('href', url);
            link.setAttribute('download', `practical_assessment_${selectedSubject || 'all'}_${format(new Date(), 'yyyyMMdd')}.csv`);
            link.style.visibility = 'hidden';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
        ...
        <div className="container mx-auto py-8 space-y-8">
            ...
            <Card className="border-t-4 border-t-blue-500 shadow-md">
                <CardHeader>
                    ...
                    <CardDescription>Choose subject for practical assessment.</CardDescription>
                </CardHeader>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={selectedYear} onValueChange={setSelectedYear} disabled={loadingFilters}>
                                ...
                                <SelectContent>
                                    {filterOptions?.academicYears?.filter((year: any) => {
                                        const yearValue = year.AcademicYear || year;
                                        return yearValue === "2025-2026";
                                    }).map((year: any, idx: number) => (
                                        <SelectItem key={idx} value={year.AcademicYear || year}>
                                            {year.AcademicYear || year}
                                        </SelectItem>
                                    ))}
                                    {!filterOptions?.academicYears && <SelectItem value="2025-2026">2025-2026 (Fallback)</SelectItem>}
                                </SelectContent>
                            </Select>
                        </div>
