    ...
    const fetchSubjects = async (
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('[lecture-plan] Departments:', depts.length, '| Own dept:', deptId, '| Other depts:', otherDeptIds);
            }

            // 1. User's own department
            const ownDeptData = await fetchWorkloadForDept(deptId);
            const ownFiltered = applyYearSemesterFilter(ownDeptData);
            if (process.env.NODE_ENV === 'development') {
                console.log('[lecture-plan] Own dept workload: raw', ownDeptData.length, '| after year/sem filter', ownFiltered.length);
            }
            ...
            for (const otherId of otherDeptIds) {
                ...
                if (process.env.NODE_ENV === 'development' && otherFiltered.length) {
                    console.log(`[lecture-plan] Other dept ${otherId} (${getDeptName(otherId)}):`, otherFiltered.length, 'subjects');
                }
                otherDeptSubjectCount += otherFiltered.length;
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('[lecture-plan] Subjects: own', ownFiltered.length, '| other-dept', otherDeptSubjectCount, '| merged unique', merged.length);
            }
            setSubjects(merged);
