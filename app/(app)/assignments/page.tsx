...
const EXPECTED_CSV_HEADERS = ['type', 'proposed_date', 'actual_date', 'date_returned', 'remarks']
...
export default function AssignmentsPage() {
        ...
        Papa.parse(file, {
            ...
            complete: async (results) => {
                ...
                const validRows = rows.map(row => {
                    const proposed = parseCSVDateToAppTz(row.proposed_date || row.ProposedDate || '')
                    ...
                    return item
                }).filter(row => row.proposed_date)

                if (validRows.length > 0) {
                    // Using upsert to handle both inserts and updates
                    const { error } = await supabase.from('assignments').upsert(validRows)
    ...
    const handleAssignmentFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setEditingItem(prev => ({ ...prev, file_url: publicUrl }))
        } catch (error: any) {
    ...
    const downloadSample = () => {
        const csvContent = `type,proposed_date,actual_date,date_returned,remarks\nAssignment,2024-02-15,2024-02-15,2024-02-20,Chapter 1`
    ...
    const handleExport = () => {
        ...
        if (link.download !== undefined) {
            const url = URL.createObjectURL(blob);
            link.setAttribute('href', url);
            link.setAttribute('download', `assignments_schedule_${showAllSubjects ? 'all' : selectedSubject}_${getTodayInAppTz('yyyyMMdd')}.csv`);
            link.style.visibility = 'hidden';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
    ...
    const handlePrint = () => {
        ...
        let printContent = `
            ...
            <html>
            <head>
                <title>Assignments / Lab Records - ${subjectName}${sectionLabel}</title>
                <style>
                    @media print {
                        @page {
                            size: A4;
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th style="width: 13%;">Actual Date</th>
                            <th style="width: 13%;">Date Returned</th>
        ...
        printContent += `
                ...
                <div class="footer">
                    <p>Total Entries: ${dataToPrint.length}</p>
                </div>
            </body>
            </html>
        `;
    ...
    const columns: ColumnDef<Assignment>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => row.original.file_url ? (
                <a href={row.original.file_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 flex items-center gap-1">
    ...
    const handleAddItem = () => {
        ...
        setEditingItem({ subject: subjObj ? subjObj.name : '', section: selectedSection || null, type: 'Assignment' })
        setOpen(true)
    }
                        ...
                        <div className="grid gap-2">
                            ...
                            {editingItem?.file_url && (
                                <div className="text-sm text-green-600 flex items-center gap-1 mt-1">
                                    ...
                                    <a href={editingItem.file_url} target="_blank" rel="noopener noreferrer" className="hover:underline">
                                        File Attached
                                    </a>
                                </div>
                            )}
