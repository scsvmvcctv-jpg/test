...
export default function AssignmentsPage() {
    ...
    const handlePrint = () => {
        ...
        let printContent = `
            ...
            <html>
            <head>
                <title>Assignments / Lab Records - ${subjectName}</title>
                <style>
                    @media print {
                        @page {
                            size: A4;
        ...
        dataToPrint.forEach((item, index) => {
            ...
            const actualDate = item.actual_date ? format(new Date(item.actual_date), 'dd/MM/yyyy') : '-';
            const returnedDate = item.date_returned ? format(new Date(item.date_returned), 'dd/MM/yyyy') : '-';
